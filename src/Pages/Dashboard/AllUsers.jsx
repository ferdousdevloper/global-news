...
const AllUsers = () => {
  ...
  const { data: users = [], refetch, isLoading, error } = useQuery({
    queryKey: ["allUsers"],
    queryFn: async () => {
      const { data } = await axios.get("global-news-server-phi.vercel.app/users"); // Update the URL if necessary
      return data;
    }
  });
  ...
  const handleMakeAdmin = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch(`global-news-server-phi.vercel.app/users/admin/${user._id}`).then((res) => {
  ...
  const handleMakeBlock = (user) => {
    axios.patch(`global-news-server-phi.vercel.app/users/block/${user._id}`).then((res) => {
      console.log(res.data);
      if (res.data.modifiedCount > 0) {
        refetch();
  ...
  const handleMakeActive = (user) => {
    axios.patch(`global-news-server-phi.vercel.app/users/active/${user._id}`).then((res) => {
      console.log(res.data);
      if (res.data.modifiedCount > 0) {
        refetch();
  ...
  const handleDeleteUser = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`global-news-server-phi.vercel.app/users/${user._id}`).then((res) => {
