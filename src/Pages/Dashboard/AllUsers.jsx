...
const AllUsers = () => {
  ...
  const { data: users = [], refetch, isLoading, error } = useQuery({
    queryKey: ["allUsers"],
    queryFn: async () => {
      const { data } = await axios.get("http://localhost:3001p/users"); // Update the URL if necessary
      return data;
    }
  });
  ...
  const handleMakeAdmin = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.patch(`http://localhost:3001p/users/admin/${user._id}`).then((res) => {
  ...
  const handleMakeBlock = (user) => {
    axios.patch(`http://localhost:3001p/users/block/${user._id}`).then((res) => {
      console.log(res.data);
      if (res.data.modifiedCount > 0) {
        refetch();
  ...
  const handleMakeActive = (user) => {
    axios.patch(`http://localhost:3001p/users/active/${user._id}`).then((res) => {
      console.log(res.data);
      if (res.data.modifiedCount > 0) {
        refetch();
  ...
  const handleDeleteUser = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`http://localhost:3001p/users/${user._id}`).then((res) => {
