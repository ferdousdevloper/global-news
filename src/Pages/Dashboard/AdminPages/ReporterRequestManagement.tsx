...
const ReporterRequestManagement: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const response = await fetch("global-news-server-phi.vercel.app/pending-reporter-requests");
  ...
  const onConfirmAction = async (confirmed: boolean) => {
    if (confirmed && selectedUser && action) {
      const response = await fetch("global-news-server-phi.vercel.app/admin/approve-request", {
        method: "PATCH",
