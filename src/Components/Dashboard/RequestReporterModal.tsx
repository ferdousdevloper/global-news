...
const RequestReporterModal: React.FC<RequestReporterModalProps> = ({
  ...
  useEffect(() => {
    if (email && isOpen) {
      ...
      const fetchRequestStatus = async () => {
        const response = await fetch(`global-news-server-phi.vercel.app/request-status?email=${email}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await fetch("global-news-server-phi.vercel.app/request-reporter", {
      method: "POST",
      headers: {
