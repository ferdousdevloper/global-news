...
const RequestReporterModal: React.FC<RequestReporterModalProps> = ({
  ...
  useEffect(() => {
    if (email && isOpen) {
      ...
      const fetchRequestStatus = async () => {
        const response = await fetch(`https://global-news-server-five.vercel.app/request-status?email=${email}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const response = await fetch("https://global-news-server-five.vercel.app/request-reporter", {
      method: "POST",
      headers: {
