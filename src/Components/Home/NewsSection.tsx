          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {allNews.map((item) => (
              <div key={item.id} className="border p-4 rounded-lg shadow-lg">
                ...
                <p className="text-sm mb-2 text-gray-600">{new Date(item.date_time).toLocaleDateString()}</p>
          ...
          <div className="space-y-6">
            {latestNews.map((item) => (
              <div key={item.id} className="border p-4 rounded-lg shadow-lg">
                ...
                <p className="text-sm text-gray-600">{new Date(item.date_time).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
          <button className="mt-6 bg-[#02AA08] text-white px-4 py-2 rounded hover:bg-[#028A06]">
